...
use nginx_sys::{
    ...
    ngx_shm_zone_t, ngx_str_t, ngx_uint_t, NGX_CONF_TAKE2, NGX_ERROR, NGX_HTTP_MAIN_CONF,
    NGX_HTTP_MAIN_CONF_OFFSET, NGX_HTTP_MODULE, NGX_HTTP_VAR_CHANGEABLE, NGX_HTTP_VAR_NOCACHEABLE,
    NGX_LOG_EMERG, NGX_OK,
};
use ngx::core::{NgxStr, NgxString, Pool, RbTree, SlabPool, Status, NGX_CONF_ERROR, NGX_CONF_OK};
...
static NGX_HTTP_SHARED_DICT_MODULE_CTX: ngx_http_module_t = ngx_http_module_t {
    ...
    postconfiguration: Some(HttpSharedDictModule::postconfiguration),
...
type SharedData = ngx::sync::RwLock<RbTree<NgxString<SlabPool>, NgxString<SlabPool>, SlabPool>>;
...
fn ngx_http_shared_dict_get_shared(shm_zone: &mut ngx_shm_zone_t) -> Result<&SharedData, Status> {
    ...
    if alloc.as_mut().data.is_null() {
        let shared: RbTree<NgxString<SlabPool>, NgxString<SlabPool>, SlabPool> =
            RbTree::try_new_in(alloc.clone()).map_err(|_| Status::NGX_ERROR)?;
...
extern "C" fn ngx_http_shared_dict_add_variable(
    ...
    if unsafe { nginx_sys::ngx_http_compile_complex_value(&mut ccv) } != NGX_OK as _ {
        return NGX_CONF_ERROR;
    }
...
extern "C" fn ngx_http_shared_dict_get_variable(
    ...
    if unsafe { ngx_http_complex_value(r, data as _, &mut key) } != NGX_OK as _ {
...
extern "C" fn ngx_http_shared_dict_set_variable(
    ...
    let Ok(value) = NgxString::try_from_bytes_in(value, alloc.clone()) else {
        return;
    };
