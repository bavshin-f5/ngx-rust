...
use ngx::r#async::{self as ngx_async, http::RequestBody};
...
impl Timer {
    ...
    unsafe extern "C" fn timer_handler(ev: *mut ffi::ngx_event_t) {
        let off = mem::offset_of!(Timer, ev) as isize;
    ...
    ) -> Poll<Result<(), core::convert::Infallible>> {
        if self.ev.timedout() != 0 {
            Poll::Ready(Ok(()))
        } else if self.ev.timer_set() != 0 {
            Poll::Pending
        } else {
            unsafe { ffi::ngx_add_timer(&mut self.ev, duration) };
...
unsafe extern "C" fn ngx_http_async_req_body_handler(r: *mut ngx_http_request_t) {
    ...
    let task = ngx_async::runtime::spawn(async move {
        ...
        if let Err(err) = async {
            ...
            let mut timer = pin!(Timer::new(req.connection()));
            future::poll_fn(|cx| timer.poll_sleep(1000, cx)).await?;
