...
fn schedule(runnable: Runnable, info: ScheduleInfo) {
    if info.woken_while_running {
        ...
        ngx_log_debug!(unsafe { (*ngx_cycle).log }, "runtime::run failed {:?}", err);
    }
}
...
extern "C" fn ngx_async_posted_event_handler(_ev: *mut ngx_event_t) {
    ngx_log_debug!(unsafe { (*ngx_cycle).log }, "runtime::step enter");
    QUEUE.with(|(_, receiver)| {
        while let Ok(runnable) = receiver.try_recv() {
            ngx_log_debug!(unsafe { (*ngx_cycle).log }, "runtime::step iter");
            let _ignore = catch_unwind(|| runnable.run());
        }
    });
    ngx_log_debug!(unsafe { (*ngx_cycle).log }, "runtime::step exit");
}
